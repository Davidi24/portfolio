...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  className,
  menuButtonColor = '#C6F435',
  openMenuButtonColor = '#C6F435',
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach((ls, i) => {
      tl.fromTo(ls.el, { xPercent: ls.start }, { xPercent: 0, duration: 0.45, ease: 'power3.inOut' }, i * 0.09);
    });
    const lastTime = layerStates.length ? (layerStates.length - 1) * 0.09 : 0;
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    spinTweenRef.current = gsap.to(icon, opening
  ...
  const prelayerColors = colors.slice(0, 5);
  ...
  return (
    <div
      ...
      style={{
        ...
        ...(menuButtonBackground ? { ['--sm-toggle-bg' as string]: menuButtonBackground } : {}),
      }}
    ...
    >
      ...
      <header className="staggered-menu-header">
        <button
          ...
          type="button"
        >
          <span ref={textInnerRef === null ? undefined : undefined} className="sm-toggle-textWrap" aria-hidden="true">
      ...
      <aside ref={panelRef} className="staggered-menu-panel" aria-hidden={!open} style={panelColor ? { background: panelColor } : undefined}>
